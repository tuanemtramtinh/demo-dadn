...
export class Chapter2 {
  ...
  public section3_3 = (usH) => {
    ...
    const aw =
      Ka *
      (u_II_III + 1) *
      ((T_2 * KHB) / (usH ** 2 * u_II_III * ba)) ** (1 / 3); //Khoảng cách trục
    ...
    const dw1 =
      Kd *
      ((T_2 * KHB * (u_II_III + 1)) / (usH ** 2 * u_II_III * bd)) ** (1 / 3); //Đường kính vòng lăn bánh nhỏ
  ...
  public section3_4 = (aw: number) => {
    const kyValues = [
      0.008, 0.032, 0.063, 0.114, 0.178, 0.243, 0.318, 0.41, 0.51, 0.622, 0.74,
      0.87, 1, 1.145, 1.295, 1.45, 1.622, 1.792, 1.985, 2.16, 2.34, 2.53, 2.742,
      2.94, 3.155, 3.38, 3.605, 3.835, 4.065, 4.29, 4.54, 4.785, 5.03, 5.28,
      5.52, 5.79, 6.05, 6.315, 6.585, 6.86, 7.14, 7.42, 7.7, 8, 8.29, 8.59,
      8.885, 9.175, 9.46, 9.765,
    ];
    ...
    const xt = x1 + x2;
    const kx = (1000 * xt) / zt;
    let ky = kyValues.find((value, idx) => kx < idx + 1);
    ...
    const atw_test =
      (zt * m * Math.cos((alpha * Math.PI) / 180)) / (2 * aw_test);
    ...
    const epsilona =
      (1.88 - 3.2 * (1 / z1_rounded + 1 / z2_rounded)) * Math.cos(B); //Hệ số trùng khớp ngang
  ...
  public section3_5 = (
    ...
    const VH = H * g0 * v_rounded * (aw / u_II_III) ** (1 / 2);

    const bw = ba * aw;
    const KHV = 1 + (VH * bw * dw1) / (2 * T_2 * KHB * KHA);
    ...
    const usHtest =
      ...
      ZH *
      Ze *
      ((2 * T_2 * KH * (u_II_III + 1)) / (bw * u_II_III * dw1 ** 2)) ** (1 / 2);
  ...
  public section3_6 = (
    ...
    let YF1: number; //hệ số dạng răng bánh 1
    if (x1 === 0.5) {
    ...
    let YF2; //hệ số dạng răng bánh 2
    if (x2 === 0.5) {
    ...
    const vF = F * g0 * v * (aw / u_II_III) ** (1 / 2);
    const KFV = 1 + (vF * bw * dw1) / (2 * T_2 * KFB * KFA);
    const KF = KFB * KFA * KFV;

    const usF1test = (2 * T_2 * KF * Yepsilone * YB * YF1) / (bw * dw1 * m);
    const usF2test = usF1test * (YF2 / YF1);
  };

  //3.7. Kiểm nghiệm độ bền quá tải
  public section3_7 = (usH: number, usF1: number, usF2: number) => {
    const Kqt = 1;
  ...
  public section3 = () => {
    ...
    const { usH, usF1, usF2 } = this.section3_2(B1, B2);
    const { KHB, ba, aw } = this.section3_3(usH);
    const {
      m,
      at,
      atw,
